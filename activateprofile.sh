...
{	
	echo "Reading Default Profile 0"  >> /home/pi-star/ActivateProfile.txt
                ...
                m9=$(sed -nr "/^\[Profile $pnum\]/ { :l /^Port[ ]*=/ { s/.*=[ ]*//; p; q;}; n; b l;}" $fromfile)
                m10=$(sed -nr "/^\[Profile $pnum\]/ { :l /^Password[ ]*=/ { s/.*=[ ]*//; p; q;}; n; b l;}" $fromfile)
                m11=$(sed -nr "/^\[Profile $pnum\]/ { :l /^TalkGroup[ ]*=/ { s/.*=[ ]*//; p; q;}; n; b l;}" $fromfile)

		ms6=${"$m6":0:7}" 
                m27=$(sed -nr "/^\[Profile $pnum\]/ { :l /^Password[ ]*=/ { s/.*=[ ]*//; p; q;}; n; b l;}" $fromfile)
...
sudo mount -o remount,rw /
	...
	echo "Starting Set Defaults $1" >> /home/pi-star/ActivateProfile.txt
		 ...
		 sudo sed -i '/^\[/h;G;/General/s/\(Id=\).*/\1'"$ms6"'/m;P;d' /etc/mmdvmhost.tmp
...
sudo mount -o remount,rw /
        sudo sed -i '/^\[/h;G;/General/s/\(^Id=\).*/\1'"$ms6"'/m;P;d' /etc/mmdvmhost.tmp
	sudo sed -i '/^\[/h;G;/DMR/s/\(^Id=\).*/\1'"$ms6"'/m;P;d' /etc/mmdvmhost.tmp
    #    sudo sed -i '/^\[/h;G;/DMR]/s/\(^Id=\).*/\1'"$ms6"'/m;P;d' /etc/mmdvmhost.tmp
...
function setysfgateway
{
		 ...
		 sudo sed -i '/^\[/h;G;/General/s/\(^Id=\).*/\1'"$ms6"'/m;P;d' /etc/ysfgateway
...
echo "Finished DMR Gateway Script"
	...
	if [ "$m7" = 'DMR2YSF' ]; then
		setdmr
		 ...
		 sudo sed -i '/^\[/h;G;/DMR Network/s/\(^Id=\).*/\1'"$ms6"'/m;P;d' /etc/dmr2ysf
...
test+=32
	...
	if [ "$m7" = 'DMR2NXDN' ]; then
		...
		sudo sed -i '/^\[/h;G;/DMR Network/s/\(^Id=\).*/\1'"$ms6"'/m;P;d' /etc/dmr2nxdn

		 sudo sed -i '/^\[/h;G;/DMR Network/s/\(^Port=\).*/\162035/m;P;d' /etc/mmdvmhost.tmp
		 sudo sed -i '/^\[/h;G;/DMR Network/s/\(^Local=\).*/\162034/m;P;d' /etc/mmdvmhost.tmp
		 sudo sed -i '/^\[/h;G;/DMR Network/s/\(^Password=\).*/\1'"none"'/m;P;d' /etc/mmdvmhost.tmp
		 sudo sed -i '/^\[/h;G;/Enabled/s/\(Enabled=\).*/\11/m;P;d' /etc/dmr2nxdn
	sudo sed -i '/^\[/h;G;/DMR Network/s/\(StartupDstId=\).*/\1'"$m11"'/m;P;d' /etc/dmr2nxdn
	...
	if [ "$m7" = 'YSF2DMR' ]; then
		 ...
		 sudo sed -i '/^\[/h;G;/DMR Network/s/\(StartupDstId=\).*/\1'"$m11"'/m;P;d' /etc/ysf2dmr
		 ...
		 sudo sed -i '/^\[/h;G;/DMR Network/s/\(^Id=\).*/\1'"$ms6"'/m;P;d' /etc/ysf2dmr
...
test+=32
	...
	if [ "$m7" = 'YSF2NXDN' ]; then
		 ...
		 sudo sed -i '/^\[/h;G;/NXDN Network/s/\(StartupDstId=\).*/\1'"$m11"'/m;P;d' /etc/ysf2nxdn
		 ...
		 sudo sed -i '/^\[/h;G;/NXDN Network/s/\(^Id=\).*/\1'"$ms6"'/m;P;d' /etc/ysf2nxdn
...
setysf
		 ...
		 sudo sed -i '/^\[/h;G;/General/s/\(^Id=\).*/\1'"$ms6"'/m;P;d' /etc/mmdvmhost
...
test+=32
	...
	if [ "$m7" = 'YSF2P25' ]; then
		 ...
		 sudo sed -i '/^\[/h;G;/P25 Network/s/\(StartupDstId=\).*/\1'"$m11"'/m;P;d' /etc/ysf2p25
		 sudo sed -i '/^\[/h;G;/P25 Network/s/\(^Address=\).*/\1'"$m8"'/m;P;d' /etc/ysf2p25
		 sudo sed -i '/^\[/h;G;/P25 Network/s/\(^Port=\).*/\1'"$m10"'/m;P;d' /etc/ysf2p25
		 sudo sed -i '/^\[/h;G;/P25 Network/s/\(^Id=\).*/\1'"$ms6"'/m;P;d' /etc/ysf2p25
